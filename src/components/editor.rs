	...
	view! { cx,
		<div role="radiogroup" aria-label="Werkzeug auswählen" class="p-6 flex w-full max-w-sm">
			<button
				role="radio"
				aria-label="Pinsel 1"
				class="bg-primary text-primaryText"
				on:click=move |_| brush.set(TileColor::Primary)
			>
				<box-icon name="brush" size="md" color="currentColor" />
			</button>
			<button
				role="radio"
				aria-label="Pinsel 1"
				class="bg-secondary text-secondaryText"
				on:click=move |_| brush.set(TileColor::Secondary)
			>
				<box-icon name="brush" size="md" color="currentColor" />
			</button>
			<button
				role="radio"
				aria-label="Pinsel 1"
				class="bg-background text-backgroundText"
				on:click=move |_| brush.set(TileColor::None)
			>
				<box-icon name="eraser" size="md" color="currentColor" />
			</button>
		</div>
